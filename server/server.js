...
const DB_ENCRYPTION_SECRET = String(process.env.FAMILY_DB_ENCRYPTION_KEY || "");
const TURN_SHARED_SECRET = String(process.env.TURN_SHARED_SECRET || "");
const TURN_URLS = String(process.env.TURN_URLS || "").split(",").map(value => value.trim()).filter(Boolean);

function databaseEncryptionKey() {
    return DB_ENCRYPTION_SECRET
        ? crypto.createHash("sha256").update(DB_ENCRYPTION_SECRET).digest()
        : null;
}
...
function decodeDatabase(raw) {
    ...
    const key = databaseEncryptionKey();
    if (!key) {
        const error = new Error("Encrypted database requires FAMILY_DB_ENCRYPTION_KEY");
    ...
    try {
        ...
        const error = new Error("Unable to decrypt database: check FAMILY_DB_ENCRYPTION_KEY");
        error.code = "DATABASE_DECRYPT_FAILED";
        throw error;
    }
}

function encodeDatabase(db) {
    const key = databaseEncryptionKey();
    ...
    return JSON.stringify({encrypted:true,version:1,iv:iv.toString("base64"),tag:cipher.getAuthTag().toString("base64"),data:data.toString("base64")});
}

function callIceServers(userId) {
    const servers = [{urls:["stun:stun.l.google.com:19302","stun:stun1.l.google.com:19302"]}];
    if (!TURN_SHARED_SECRET || !TURN_URLS.length) return servers;
    const ttl = Math.max(300, Math.min(86400, Number(process.env.TURN_TTL_SECONDS) || 3600));
    const username = `${Math.floor(Date.now() / 1000) + ttl}:${Number(userId)}`;
...
function loadDatabase() {
    try {
        const db = fs.existsSync(DATA_FILE)
            ? decodeDatabase(fs.readFileSync(DATA_FILE, "utf8"))
            : defaultDatabase();
        ...
        if (e.code === "ENCRYPTION_KEY_REQUIRED" || e.code === "DATABASE_DECRYPT_FAILED") throw e;
...
function buildMessage(user, text, audio, media) {
    ...
    if (audio) {
        ...
        const match=data.match(/^data:(image|video)\/([a-zA-Z0-9.+-]+);base64,([A-Za-z0-9+/=]+)$/);
        if(!match) throw new Error("Недопустимый формат медиа");
        const type=match[1];
        const mime=`${match[1]}/${match[2]}`.toLowerCase();
        const bytes=Math.floor(match[3].length*3/4)-(match[3].endsWith("==")?2:match[3].endsWith("=")?1:0);
        const limit=type==="video"?6*1024*1024:3*1024*1024;
        if(bytes>limit) throw new Error(type==="video"?"Видео слишком большое (максимум 6 МБ)":"Фото слишком большое (максимум 3 МБ)");
        message.type=type==="video"?"video":"photo";
        message.media={id:String(media.id||audioId()),mime,data,name:String(media.name||"").slice(0,255),size:bytes,pending:[]};
        message.media.caption=String(media.caption||"").trim().slice(0,2000);
    } else {
        message.type = "text";
...
function messageRecipients(scope, key) {
    if (scope === "global") return (db.familyChatMembers || []).map(Number);
    if (scope === "group") {
        const group = (db.groups || []).find(item => Number(item.id) === Number(key));
        return group ? (group.memberIds || []).map(Number) : [];
    }
    if (scope === "private") {
        const ids = String(key).split("_").map(Number).filter(Number.isFinite);
        return ids.length === 2 ? ids : [];
    }
    return [];
}

function sendToRecipients(userIds, payload) {
    for (const userId of [...new Set(userIds.map(Number))]) sendToUserSockets(userId, payload);
}

function sendToMessageRecipients(scope, key, payload) {
    sendToRecipients(messageRecipients(scope, key), payload);
}

function limitMessages(messages, rawLimit) {
            ...
            try {
                ...
                if (
                    ...
                    if (db.users.length >= maxUsers) {
                        sendJson(res,400,{success:false,error:`Можно создать не более ${maxUsers} пользователей`});
                ...
                if (req.method === "GET" && url.pathname === "/api/chats") {
                    ...
                    if (globalLast && (db.familyChatMembers||[]).map(Number).includes(Number(user.id))) {
                        ...
                        chats.push({scope:"global", id:"global", name:"Общий чат", unread:count, lastMessage:globalLast});
                ...
                if(groupMatch){
                    ...
                    if(req.method==="PUT"){
                        if(Number(group.ownerId)!==Number(user.id) && user.role!=="admin"){sendJson(res,403,{success:false,error:"Управлять группой может только её создатель"});return;}
                        const name=String(body.name||group.name).trim().slice(0,80);
                        const requested=Array.isArray(body.memberIds)?body.memberIds.map(Number):group.memberIds;
                        const memberIds=[...new Set([Number(group.ownerId),...requested].filter(Boolean))];
                        if(!name){sendJson(res,400,{success:false,error:"Введите название группы"});return;}
                        if(!memberIds.every(memberId=>db.users.some(item=>Number(item.id)===memberId))){sendJson(res,400,{success:false,error:"Некорректный участник группы"});return;}
                        const previous=group.memberIds||[];
                        group.name=name;group.memberIds=memberIds;group.updatedAt=new Date().toISOString();saveDatabase(db);
                        const publicGroup={...group,members:memberIds.map(memberId=>publicUser(db.users.find(item=>Number(item.id)===Number(memberId))||{}))};
                        sendToRecipients([...previous,...memberIds],{type:"group_updated",group:publicGroup});
                        sendJson(res,200,{success:true,group:publicGroup});return;
                    }

                    if(req.method==="DELETE"){
                        if(Number(group.ownerId)!==Number(user.id) && user.role!=="admin"){sendJson(res,403,{success:false,error:"Удалить группу может только её создатель"});return;}
                        const members=group.memberIds||[];db.groups=db.groups.filter(item=>Number(item.id)!==id);delete db.groupChats[String(id)];saveDatabase(db);
                        sendToRecipients(members,{type:"group_deleted",groupId:id});sendJson(res,200,{success:true});return;
                    }

                    if(req.method==="GET"){
                        const messages=limitMessages(db.groupChats?.[String(id)]||[],url.searchParams.get("limit"));
                    ...
                    if(req.method==="POST"){
                        ...
                        chat.push(message);
                        saveDatabase(db);
                        sendToRecipients(group.memberIds||[],{type:"group_message",groupId:id,message});
                ...
                if (
                    ...
                    sendJson(res,200,{success:true,messages:limitMessages(db.globalChat,url.searchParams.get("limit"))});
                ...
                if (
                    ...
                    sendToRecipients(db.familyChatMembers||[],{type:"global_message",message});
                ...
                if (
                    ...
                    if (
                        ...
                        sendJson(
                            ...
                            {
                                success: true,
                                messages: limitMessages(db.privateChats[id],url.searchParams.get("limit"))
                            }
                        );
                ...
                const messageActionMatch = url.pathname.match(/^\/api\/messages\/(global|private|group)\/([^/]+)\/(\d+)$/);
                if (messageActionMatch && (req.method === "PUT" || req.method === "DELETE")) {
                    ...
                    const recipients=messageRecipients(kind,key);
                    if(!recipients.includes(Number(user.id))){sendJson(res,403,{success:false,error:"Нет доступа к сообщению"});return;}
                    const chat=kind === "global" ? db.globalChat : kind === "group" ? (db.groupChats?.[key] || []) : (db.privateChats[key] || []);
                    ...
                    if (req.method === "PUT") {
                        ...
                        sendToMessageRecipients(kind,key,{type:"message_updated",scope:kind,key,message}); sendJson(res,200,{success:true,message}); return;
                    }
                    ...
                    sendToMessageRecipients(kind,key,{type:"message_deleted",scope:kind,key,messageId}); sendJson(res,200,{success:true}); return;
                }
                ...
                if (req.method === "POST" && url.pathname === "/api/audio/ack") {
                    ...
                    if(!messageRecipients(scope,key).includes(Number(user.id))){sendJson(res,403,{success:false,error:"Нет доступа к сообщению"});return;}
                ...
                const reactionMatch =
                    url.pathname.match(
                        /^\/api\/messages\/(global|private|group)\/([^/]+)\/(\d+)\/reaction$/
                    );
                ...
                if (
                    ...
                    sendToMessageRecipients(kind,key,{
                ...
                // ==========================================
                // CALL ICE CONFIGURATION
                // ==========================================
                if (req.method === "GET" && url.pathname === "/api/calls/ice") {
                    const user=authUser(req);
                    if(!user){sendJson(res,401,{success:false,error:"Не авторизован"});return;}
                    sendJson(res,200,{success:true,iceServers:callIceServers(user.id)});
                    return;
                }
    ...
    (socket, req) => {
        ...
        socket.on("message",raw=>{
            try{
                ...
                const payload={...msg,from:Number(userId),fromName:db.users.find(u=>Number(u.id)===Number(userId))?.name||"NOVA"};
