...
self.addEventListener("install", event => {
  event.waitUntil((async () => {
    ...
    for (const url of SHELL) {
      ...
      catch (e) { console.warn("MD LinkUp SW precache skipped:", url, e); }
    }
    ...
    try {
      ...
      if (res.ok) {
        try {
          ...
          console.warn("MD LinkUp SW cache:", e);
        }
      }
      return res;
    } catch (e) {
...
self.addEventListener("push", event => {
  ...
  catch { data = {title:"MD LinkUp", body:event.data?.text() || "Новое сообщение"}; }
  event.waitUntil(self.registration.showNotification(data.title || "MD LinkUp", {
    body: data.body || "Новое сообщение",
    icon: "./assets/icon-180.png",
    badge: "./assets/icon-180.png",
    tag: data.tag || "mdlinkup-message",
    renotify: true,
